#!/usr/bin/env python3
"""
IC-Weighted Multi-Factor Strategy Backtest

Backtests the recommended IC-weighted strategy from multi-factor analysis:
...
# IC-Weighted Factor Configuration (from MULTI_FACTOR_ANALYSIS_REPORT.md)
FACTOR_WEIGHTS = {
    'Operating_Profit_Margin': 0.35,  # Top IC performer (IC=0.086)
    'ROE_Proxy': 0.25,                # #2 IC performer (IC=0.091)
    'PE_Ratio': 0.20,                 # Value factor (IC=0.051)
    '12M_Momentum': 0.15,             # Momentum factor (IC=0.043)
    'PB_Ratio': 0.05                  # Secondary value (IC=0.031)
}
...
def get_top_stocks_ic_weighted(
    ...
    """
    Get top stocks by IC-weighted composite factor score
    ...
    df_pivot['composite_score'] = 0.0
    for factor, weight in factor_weights.items():
        if factor in df_pivot.columns:
            df_pivot['composite_score'] += df_pivot[factor] * weight

    # Select top percentile
    top_tickers = df_pivot[df_pivot['composite_score'] >= top_percentile].index.tolist()

    logger.info(f"  Selected {len(top_tickers)} stocks (IC-weighted score >= {top_percentile}%ile)")
    return top_tickers
...
def backtest_ic_weighted_strategy(
    db: PostgresDatabaseManager,
    start_date: str,
    end_date: str,
    factor_weights: dict,
    ...
    """
    Backtest IC-weighted multi-factor strategy with transaction costs
    ...
    """
    logger.info("=" * 80)
    logger.info("IC-WEIGHTED MULTI-FACTOR STRATEGY BACKTEST")
    logger.info("=" * 80)
    ...
    logger.info("")
    logger.info("Factor Weights (IC-Weighted):")
    for factor, weight in factor_weights.items():
    ...
    for i, date in enumerate(prices.index):
        ...
        if date in rebalance_dates:
            ...
            # Get top tickers based on IC-weighted factor scores
            top_tickers = get_top_stocks_ic_weighted(
...
Examples:
  ...
  python3 scripts/backtest_ic_weighted_strategy.py \\
    ...
    # Run backtest
    portfolio_df = backtest_ic_weighted_strategy(
        db=db,
