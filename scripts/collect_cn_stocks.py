#!/usr/bin/env python3
"""
collect_cn_stocks.py - China Market OHLCV Data Collection Script

Purpose:
- Collect China stock OHLCV data with Stage 0-1 filtering
- Support SSE (Shanghai) and SZSE (Shenzhen) via Connect Program
- Integrate with KIS Overseas API (13x faster than AkShare)
- Apply market-specific error handling and rate limiting

Market Characteristics:
- Market Hours: 09:30-11:30, 13:00-15:00 CST (10:30-12:30, 14:00-16:00 KST)
- Trading Days: Mon-Fri (excluding China holidays)
- Lunch Break: 11:30-13:00 CST
- API Rate Limit: 20 req/sec (KIS Overseas API)
- Ticker Format: 6 digits with .SS/.SZ suffix (e.g., 600519.SS, 000858.SZ)
...
def main():
    """Main execution function for China market data collection"""
    parser = argparse.ArgumentParser(
        description='China Market OHLCV Data Collection (SSE, SZSE via Connect)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Collect all CN stocks with filtering (default: 250 days)
  python3 scripts/collect_cn_stocks.py

  # Collect specific tickers (Moutai, Wuliangye)
  python3 scripts/collect_cn_stocks.py --tickers 600519 000858

  # Collect with Stage 1 filter (technical pre-screen)
  python3 scripts/collect_cn_stocks.py --apply-stage1

  # Collect 30-day data (quick update)
  python3 scripts/collect_cn_stocks.py --days 30

  # Dry run (mock mode, no API calls)
  python3 scripts/collect_cn_stocks.py --dry-run

Market Info:
  - Trading Hours: 09:30-11:30, 13:00-15:00 CST (lunch break 11:30-13:00)
  - API Rate Limit: 20 req/sec (KIS Overseas API)
  - Access: Shanghai-Hong Kong Connect / Shenzhen-Hong Kong Connect
  - Expected Tickers: ~1,000-1,500 A-shares (Connect eligible only)
  - Collection Time: ~1-2 minutes for 1,000 tickers (with filtering)
        """
    )
    ...
    parser.add_argument(
        '--tickers',
        nargs='+',
        help='Specific tickers to collect (e.g., 600519 000858 600036)'
    )
    parser.add_argument(
        '--days',
        type=int,
    ...
    try:
        logger.info("=" * 60)
        logger.info("China Market OHLCV Data Collection")
        logger.info("=" * 60)
        logger.info(f"Database: {args.db_path}")
        logger.info(f"Region: CN (SSE, SZSE via Connect)")
        ...
        # Initialize KIS data collector for CN market
        collector = KISDataCollector(db_path=args.db_path, region='CN')
        ...
        logger.info("")
        logger.info("=" * 60)
        logger.info("📊 China Market Collection Summary")
        logger.info("=" * 60)
        ...
        if total_success > 0:
            logger.info("")
            logger.info(f"✅ CN market data collection completed successfully!")
            ...
            logger.warning("   3. Market hours (CN: 09:30-11:30, 13:00-15:00 CST)")
            return 1
        ...
        logger.error("")
        logger.error(f"❌ CN market data collection failed: {e}")
        import traceback
        traceback.print_exc()
        return 1
