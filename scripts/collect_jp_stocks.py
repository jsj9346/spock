#!/usr/bin/env python3
"""
collect_jp_stocks.py - Japan Market OHLCV Data Collection Script

Purpose:
- Collect Japan stock OHLCV data with Stage 0-1 filtering
- Support TSE (Tokyo Stock Exchange)
- Integrate with KIS Overseas API (20x faster than yfinance)
- Apply market-specific error handling and rate limiting

Market Characteristics:
- Market Hours: 09:00-11:30, 12:30-15:00 JST (09:00-11:30, 12:30-15:00 KST)
- Trading Days: Mon-Fri (excluding Japan holidays)
...
def main():
    """Main execution function for Japan market data collection"""
    parser = argparse.ArgumentParser(
        description='Japan Market OHLCV Data Collection (TSE)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Collect all JP stocks with filtering (default: 250 days)
  python3 scripts/collect_jp_stocks.py

  # Collect specific tickers (Toyota, SoftBank)
  python3 scripts/collect_jp_stocks.py --tickers 7203 9984

  # Collect with Stage 1 filter (technical pre-screen)
  python3 scripts/collect_jp_stocks.py --apply-stage1

  # Collect 30-day data (quick update)
  python3 scripts/collect_jp_stocks.py --days 30

  # Dry run (mock mode, no API calls)
  python3 scripts/collect_jp_stocks.py --dry-run
...
Market Info:
  ...
  - Collection Time: ~30-60 seconds for 500 tickers (with filtering)
    ...
    parser.add_argument(
        '--tickers',
        nargs='+',
        help='Specific tickers to collect (e.g., 7203 9984 6758)'
    )
    parser.add_argument(
        '--days',
        type=int,
    ...
    try:
        logger.info("=" * 60)
        logger.info("Japan Market OHLCV Data Collection")
        logger.info("=" * 60)
        logger.info(f"Database: {args.db_path}")
        logger.info(f"Region: JP (TSE)")
        ...
        # Initialize KIS data collector for JP market
        collector = KISDataCollector(db_path=args.db_path, region='JP')
        ...
        logger.info("")
        logger.info("=" * 60)
        logger.info("📊 Japan Market Collection Summary")
        logger.info("=" * 60)
        ...
        if total_success > 0:
            logger.info("")
            logger.info(f"✅ JP market data collection completed successfully!")
            ...
            logger.warning("   3. Market hours (JP: 09:00-11:30, 12:30-15:00 JST)")
            return 1
        ...
        logger.error("")
        logger.error(f"❌ JP market data collection failed: {e}")
        import traceback
        traceback.print_exc()
        return 1
