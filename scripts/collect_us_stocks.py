#!/usr/bin/env python3
"""
collect_us_stocks.py - US Market OHLCV Data Collection Script

Purpose:
- Collect US stock OHLCV data with Stage 0-1 filtering
- Support NYSE, NASDAQ, AMEX markets
- Integrate with KIS Overseas API (240x faster than Polygon.io)
- Apply market-specific error handling and rate limiting

Market Characteristics:
- Market Hours: 09:30-16:00 EST (23:30-06:00 KST next day)
- Trading Days: Mon-Fri (excluding US holidays)
...
def main():
    """Main execution function for US market data collection"""
    parser = argparse.ArgumentParser(
        description='US Market OHLCV Data Collection (NYSE, NASDAQ, AMEX)',
...
Examples:
  ...
  python3 scripts/collect_us_stocks.py --days 30

  # Dry run (mock mode, no API calls)
  python3 scripts/collect_us_stocks.py --dry-run
...
Market Info:
  ...
  - Collection Time: ~2-3 minutes for 3,000 tickers (with filtering)
    ...
    parser.add_argument(
        '--tickers',
        nargs='+',
        help='Specific tickers to collect (e.g., AAPL MSFT GOOGL)'
    )
    parser.add_argument(
        '--days',
        type=int,
    ...
    try:
        logger.info("=" * 60)
        logger.info("US Market OHLCV Data Collection")
        logger.info("=" * 60)
        logger.info(f"Database: {args.db_path}")
        logger.info(f"Region: US (NYSE, NASDAQ, AMEX)")
        ...
        # Initialize KIS data collector for US market
        ...
        logger.info("")
        logger.info("=" * 60)
        logger.info("📊 US Market Collection Summary")
        logger.info("=" * 60)
        ...
        if total_success > 0:
            logger.info("")
            logger.info(f"✅ US market data collection completed successfully!")
            ...
            logger.warning("   3. Market hours (US: 09:30-16:00 EST)")
            return 1
