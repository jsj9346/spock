#!/usr/bin/env python3
"""
EDINET 재무 데이터 백필 스크립트

JP 시장 종목의 재무 데이터를 EDINET API에서 수집하여
ticker_fundamentals 테이블에 저장합니다.

지원 보고서 유형:
- annual: 有価証券報告書 (연간 보고서)
- quarterly: 四半期報告書 (분기 보고서)
- all: 연간 + 분기 모두

사용 예시:
    # 연간 데이터 백필
    python3 scripts/backfill_fundamentals_edinet.py --report-type annual --start-year 2020

    # 분기 데이터 백필 (TTM 계산용)
    python3 scripts/backfill_fundamentals_edinet.py --report-type quarterly --start-year 2023

    # 전체 데이터 백필
    python3 scripts/backfill_fundamentals_edinet.py --report-type all --limit 100
...
LOG_FILE = os.path.join(LOG_DIR, f"{datetime.now().strftime('%Y%m%d')}_backfill_fundamentals_edinet.log")
...
class EDINETFundamentalsBackfiller:
    """EDINET 재무 데이터 백필러"""

    def __init__(
        self,
        db: PostgresDatabaseManager,
        edinet_api: EDINETApiClient,
        dry_run: bool = False
    ):
        self.db = db
        self.edinet_api = edinet_api
    ...
    def get_jp_tickers(
        ...
        Args:
            ...
            period_type: 체크할 period_type (QUARTERLY 또는 ANNUAL)
        ...
        if skip_existing:
          ...
          AND t.ticker NOT IN (
              ...
              WHERE region = 'JP' AND period_type = '{period_type}'
          )
            """
    ...
    def insert_fundamental_data(self, data: EDINETFundamentalData) -> bool:
        """
    ...
    def backfill_ticker(
        ...
        try:
            ...
            if report_type in ('annual', 'all'):
                annual_data = self.edinet_api.get_historical_fundamentals(
                    ticker=ticker,
            ...
            if report_type in ('quarterly', 'all'):
                quarterly_data = self.edinet_api.get_quarterly_fundamentals(
                    ticker=ticker,
                    start_year=start_year,
                    end_year=end_year
                )
    ...
    def run_backfill(
        ...
        if tickers:
            ...
            target_tickers = self.get_jp_tickers(
                ...
                period_type="QUARTERLY" if report_type == 'quarterly' else "ANNUAL"
            )
...
사용 예시:
  ...
  # 특정 ticker만 백필
  python3 scripts/backfill_fundamentals_edinet.py --tickers 7203,9984,6758 --report-type all

  # Dry run 모드
  python3 scripts/backfill_fundamentals_edinet.py --report-type quarterly --dry-run --limit 5
        """
    )
    ...
    if not api_key:
        logger.error("❌ EDINET_API_KEY 환경 변수가 설정되지 않았습니다.")
        logger.error("   https://disclosure2.edinet-fsa.go.jp/ 에서 무료로 발급받으세요.")
        sys.exit(1)

    # EDINET API 클라이언트 초기화
    edinet_api = EDINETApiClient(
        api_key=api_key,
        rate_limit_delay=1.0,  # EDINET 권장: 1 req/sec
        use_cache=True  # 캐싱 활성화
    )

    # 연결 테스트
    if not edinet_api.validate_connection():
        logger.error("❌ EDINET API 연결 실패")
        sys.exit(1)
    logger.info("✅ EDINET API 연결 성공")

    # 백필러 초기화
    backfiller = EDINETFundamentalsBackfiller(
        db=db,
    ...
    try:
        ...
        # 캐시 통계 출력
        stats = edinet_api.get_cache_stats()
        if stats.get('enabled', False):
            logger.info(f"📊 캐시 통계: 히트={stats['hits']}, 미스={stats['misses']}, 히트율={stats['hit_rate']:.1%}")
        edinet_api.close()
        logger.info("🔒 세션 종료")
