#!/usr/bin/env python3
"""
Update Hong Kong stock lot_sizes using KIS Master File

This script updates all HK tickers in the database with accurate lot_size
information from KIS master file (hks.cod).
...
Usage:
    ...
    python3 scripts/update_hk_lot_sizes.py --tickers 0700 9988 0005
"""
...
def main():
    parser = argparse.ArgumentParser(description='Update HK stock lot_sizes')
    parser.add_argument('--dry-run', action='store_true',
                       help='Preview changes without updating database')
    parser.add_argument('--tickers', nargs='+',
                       help='Update specific tickers only (e.g., 0700 9988 0005)')
    args = parser.parse_args()

    print("="*80)
    print("Hong Kong Stock Lot_size Update Script")
    print("="*80)
    print()
    ...
    # Initialize HK adapter (will use updated _fetch_lot_size method)
    adapter = HKAdapterKIS(
        db_manager=db,
    ...
    # Get all HK tickers from database
    print("📊 Loading HK tickers from database...")
    all_tickers = get_hk_tickers_from_db(db)
    print(f"✅ Found {len(all_tickers)} HK tickers")
    print()
    ...
    if not args.dry_run:
        backup_path = f"data/backups/spock_local_before_hk_lot_size_update_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
    ...
    # Process each ticker
    print(f"🔄 Processing {stats['total']} HK tickers...")
    print("-"*80)
    ...
    for i, ticker_info in enumerate(all_tickers, 1):
        ...
        try:
            ...
            if current_lot_size == new_lot_size:
                print(f"[{i:4d}/{stats['total']}] ✓ {ticker:10s} ({name[:20]:20s}) "
    ...
    if args.dry_run:
        ...
        print()
        print("Next Steps:")
        print("1. Verify lot_size distribution matches expectations")
        print("2. Test trading engine with HK stocks")
