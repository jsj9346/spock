"""
PyKRX Fundamental Data Cache

과거 날짜의 fundamental 데이터를 캐싱하여 재조회 시 API 호출을 100% 절감합니다.

특징:
- JSON 파일 기반 캐시 (간단하고 이식성 높음)
- 과거 날짜: 영구 캐시 (데이터 변경 없음)
- 당월 날짜: 1시간 TTL (신규 데이터 반영)
- 디스크 사용량: ~50MB/년 (일 평균 ~150KB)
...
class PyKRXFundamentalCache:
    """
    PyKRX Fundamental 데이터 캐시 관리자

    일별 fundamental 데이터(PER, PBR, EPS 등)를 JSON 파일로 캐싱합니다.

    사용 예시:
        cache = PyKRXFundamentalCache()

        # 캐시 조회 (없으면 None)
        data = cache.get(date(2024, 6, 20))

        # 캐시 저장
        cache.set(date(2024, 6, 20), fundamental_data)
    ...
    DEFAULT_CACHE_DIR = "data/cache/pykrx"
    ...
    def __init__(
        ...
        Args:
            cache_dir: 캐시 디렉토리 경로 (기본: data/cache/pykrx)
            enabled: 캐시 활성화 여부
        """
        ...
        if self.enabled:
            ...
            logger.info(f"PyKRX 캐시 초기화: {self.cache_dir}")
    ...
    def _get_cache_path(self, target_date: date) -> Path:
        ...
        # 연/월 디렉토리 구조: data/cache/pykrx/2024/06/20.json
    ...
    def get(self, target_date: date) -> Optional[List[Dict]]:
        """
        캐시에서 fundamental 데이터 조회

        Args:
            target_date: 대상 날짜

        Returns:
            캐시된 fundamental 데이터 또는 None (캐시 미스)
        """
        if not self.enabled:
            return None
    ...
    def set(self, target_date: date, fundamentals: List[Dict]) -> bool:
        """
        fundamental 데이터를 캐시에 저장

        Args:
            target_date: 대상 날짜
            fundamentals: fundamental 데이터 리스트
        ...
        try:
            ...
            logger.debug(f"캐시 저장: {target_date} ({len(fundamentals)}개 종목)")
            return True
    ...
    def get_or_fetch(
        ...
        Args:
            target_date: 대상 날짜
            fetch_func: API 호출 함수 (lambda: pykrx.stock.get_market_fundamental(...))
        ...
        # API 호출
        fundamentals = fetch_func()

        # 캐시 저장 (빈 결과도 캐시하여 불필요한 재호출 방지)
        if fundamentals is not None:
            self.set(target_date, fundamentals if fundamentals else [])
...
# 글로벌 캐시 인스턴스 (싱글톤 패턴)
_global_cache: Optional[PyKRXFundamentalCache] = None


def get_pykrx_cache() -> PyKRXFundamentalCache:
    """
    글로벌 PyKRX 캐시 인스턴스 반환

    Returns:
        PyKRXFundamentalCache 싱글톤 인스턴스
    """
    global _global_cache
    if _global_cache is None:
        _global_cache = PyKRXFundamentalCache()
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("PyKRX Fundamental Cache 테스트")
    print("=" * 60)
    ...
    test_data = [
        {'ticker': '005930', 'name': '삼성전자', 'PER': 15.2, 'PBR': 1.3, 'EPS': 5000},
        {'ticker': '000660', 'name': 'SK하이닉스', 'PER': 8.5, 'PBR': 1.1, 'EPS': 12000},
    ]

    # 캐시 저장
    print(f"\n1. 캐시 저장 테스트 ({test_date})")
    result = cache.set(test_date, test_data)
    print(f"   저장 결과: {'성공' if result else '실패'}")

    # 캐시 조회
    print(f"\n2. 캐시 조회 테스트 ({test_date})")
    cached = cache.get(test_date)
    print(f"   조회 결과: {len(cached) if cached else 0}개 종목")
