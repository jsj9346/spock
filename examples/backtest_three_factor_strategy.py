...
def main():
    ...
    logger.info(
        ...
        f"   - Avg holdings: {len(signals_df) / signals_df['date'].nunique():.1f}/month"
    )
    ...
    equity_curve.to_csv(results_dir / 'three_factor_equity_curve.csv')

    # Save portfolio weights
    weights_df.to_csv(results_dir / 'three_factor_weights.csv')

    # Save rebalancing signals
    signals_df.to_csv(results_dir / 'three_factor_signals.csv', index=False)
