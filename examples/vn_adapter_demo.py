"""
Vietnam Market Adapter Demo Script

Demonstrates VNAdapter functionality for HOSE/HNX data collection:
...
Usage:
    python3 examples/vn_adapter_demo.py [--dry-run] [--max-stocks N]
...
def demo_stock_scanning(adapter: VNAdapter, max_stocks: int = 10):
    """
    Demo 1: Stock Scanning

    Demonstrates:
    - Fetching Vietnamese stock data from yfinance
    - Filtering REITs and preferred stocks
    - Saving to database with ICB→GICS sector mapping
    """
    print_section("Demo 1: Vietnam Stock Scanning (VN30 Index)")

    print(f"📊 Scanning Vietnamese stocks (max: {max_stocks})...")
    print(f"   Data source: yfinance (Yahoo Finance)")
    print(f"   Target: HOSE (Ho Chi Minh) + HNX (Hanoi)")
    ...
    try:
        ...
        print(f"✅ Successfully scanned {len(stocks)} Vietnamese stocks\n")
...
def demo_ohlcv_collection(adapter: VNAdapter, days: int = 30):
    """
    ...
    try:
        ...
        db_tickers = adapter.db.get_tickers(region='VN', asset_type='STOCK', is_active=True)

        if not db_tickers:
            print("⚠️  No Vietnamese tickers found in database. Run stock scanning first.")
            return
...
def demo_fundamentals_collection(adapter: VNAdapter):
    """
...
def demo_custom_ticker(adapter: VNAdapter, ticker: str = "VCB"):
    """
    Demo 4: Custom Ticker Management

    Demonstrates:
    - Adding custom tickers (e.g., Vietcombank: VCB)
    ...
    print(f"   Example: VCB (Vietcombank)\n")
...
def demo_database_stats(db: SQLiteDatabaseManager):
    ...
    Demonstrates:
    - Querying database for VN stocks
    ...
    try:
        # Count VN stocks
        vn_stocks = db.get_tickers(region='VN', asset_type='STOCK', is_active=True)
        print(f"📊 Vietnam Market Data Summary:")
        print(f"   Total stocks in database: {len(vn_stocks)}")
        ...
        if vn_stocks:
            ...
            query = """
                SELECT COUNT(*)
                FROM ohlcv_data
                WHERE ticker IN (SELECT ticker FROM tickers WHERE region = 'VN')
            """
...
def main():
    """Main demo execution"""
    parser = argparse.ArgumentParser(description='Vietnam Market Adapter Demo')
    ...
    print("\n" + "="*80)
    print("  Vietnam Market Adapter Demo - HOSE/HNX Stock Exchanges")
    print("="*80)
    print(f"\nConfiguration:")
    print(f"  Database: data/spock_local.db")
    print(f"  Data Source: yfinance (Yahoo Finance)")
    print(f"  Markets: HOSE (Ho Chi Minh) + HNX (Hanoi)")
    ...
    try:
        ...
        demo_custom_ticker(adapter, ticker="VCB")  # Vietcombank
        ...
        print("  5. Monitor VN30 index performance\n")
